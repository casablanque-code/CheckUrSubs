...
export default function Auth() {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'login') {
      ...
      if (error) setError(translateError(error.message));
      else       setMessage(t.auth_confirm_email);
    } else if (mode === 'reset') {
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: `${window.location.origin}?reset=1`,
      });
      if (error) setError(translateError(error.message));
      else       setMessage(t.auth_reset_sent);
    }
    setLoading(false);
  };

  const translateError = (msg) => {
    if (msg.includes('Invalid login'))           return t.auth_err_invalid;
    if (msg.includes('Email not confirmed'))     return t.auth_err_unconfirmed;
    ...
    if (msg.includes('rate limit'))              return t.auth_err_rate;
    return msg;
  };
      ...
      <div className="w-full max-w-[450px] min-h-screen border-x border-zinc-900 bg-black flex flex-col overflow-hidden">
        <div className="flex-1 flex flex-col justify-center px-8 py-12">
          ...
          <div className="mb-10 flex flex-col items-center text-center">
            ...
            <p className="text-zinc-500 text-sm mt-1">{t.auth_tagline}</p>
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={mode} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              ...
              <h2 className="text-lg font-semibold text-center mb-5">
                {mode === 'login' ? t.auth_title_login : mode === 'register' ? t.auth_title_register : t.auth_title_reset}
              </h2>
              ...
              {mode !== 'reset' && (
                <button onClick={handleGoogleLogin} disabled={loading}
                  ...
                  <GoogleIcon />
                  {mode === 'login' ? t.auth_google_login : t.auth_google_register}
                </button>
              )}
              ...
              {mode !== 'reset' && (
                <div className="flex items-center gap-3 py-1">
                  ...
                  <span className="text-zinc-600 text-xs">{t.auth_or}</span>
                  <div className="flex-1 h-px bg-zinc-800" />
                </div>
              )}
              ...
              {mode !== 'reset' && (
                <div className="relative">
                  ...
                  <input type={showPw ? 'text' : 'password'} placeholder={t.auth_password} value={password} onChange={e => setPassword(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && handleSubmit()}
              ...
              <button onClick={handleSubmit} disabled={loading || !email || (mode !== 'reset' && !password)}
                ...
                {loading
                  ...
                  : <>{mode === 'login' ? t.auth_btn_login : mode === 'register' ? t.auth_btn_register : t.auth_btn_reset} <ArrowRight className="w-4 h-4" /></>
                }
              </button>
              ...
              <div className="flex flex-col items-center gap-2 pt-2">
                {mode === 'login' && <>
                  <button onClick={() => { setMode('register'); clearState(); }} className="text-zinc-500 text-xs hover:text-zinc-300 transition">
                    {t.auth_no_account} <span className="text-zinc-300 font-medium">{t.auth_btn_register}</span>
                  </button>
                ...
                {mode === 'register' && (
                  <button onClick={() => { setMode('login'); clearState(); }} className="text-zinc-500 text-xs hover:text-zinc-300 transition">
                    {t.auth_have_account} <span className="text-zinc-300 font-medium">{t.auth_btn_login}</span>
                  </button>
                )}
                {mode === 'reset' && (
                  <button onClick={() => { setMode('login'); clearState(); }} className="text-zinc-500 text-xs hover:text-zinc-300 transition">
