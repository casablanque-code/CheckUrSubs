...
const App = ({ session, toggleLang, lang }) => {
  ...
  const {
    ...
    handleSave: saveSub, triggerDelete, undoDelete, handleImport, handleDeleteAccount,
  } = useSubscriptions(userId, session.user.email);
  ...
  const onSaveSub = async (payload) => {
    await saveSub(payload, editingSub);
  ...
  {confirmSub && (
    <motion.div
      ...
      <motion.div
        ...
        <button
          onClick={() => { if (isOnline) triggerDelete(confirmSub); setConfirmSub(null); }}
